...
export class POSCustomerOrderPage extends PageBase {
	...
	preLoadData(event?: any): void {
		...
		Promise.all([this.getMenu(), this.getTable(), this.getDeal()])
			.then((values: any) => {
	...
	async checkIPConfig(event) {
		this.pageProvider.commonService
			.connect('GET', 'POS/ForCustomer/CheckIPCustomer', {
				IDBranch: this.Table.IDBranch,
			})
			.toPromise()
			.then((resp: any) => {
				if (resp['canEdit']) {
	...
	private checkLastModifiedDate() {
		if (this.item.Id && ['Merged', 'Splitted', 'Done'].indexOf(this.item.Status) == -1) {
			this.pageProvider.commonService
				.connect('GET', 'POS/ForCustomer/CheckPOSModifiedDate', {
				...
				.then((lastModifiedDate) => {
					...
					if (lastModifiedDateText > itemModifiedDateText) {
						this.env.showMessage('Order Information has been changed, the order will be updated', 'warning');
						this.refresh();
						return true;
					}
				})
		...
		if (!this.item?.Id) {
			if (!this.Table?.IsAllowMultipleOrder) {
				...
				if (this.id != 0) {
					...
					this.env.showAlert(
						'This table already has an order. If you are not with that customer, please press the bell below to call staff',
						'Check the order and update',
						'Announcement'
					);
					this.refresh();
				}
			...
			if (this.formGroup.controls['Id'].value != 0 && this.id == 0) {
				this.env
					.showPrompt('', null, 'The order was completed', 'Create the new order', 'Close')
					.then((_) => {
						this.newOrder();
					})
					.catch((_) => {});
	...
	addToCart(item, idUoM, quantity = 1, IsUpdate = false, idx = -1) {
		if (!this.pageConfig.canEdit) {
			this.env.showMessage('Order is temporarily locked. Cannot add items!', 'warning');
			// this.isWifiSecuredModalOpen = true;
			return;
		}
		...
		if (!this.pageConfig.canEdit) {
			this.env.showMessage('Order is locked. Cannot edit or add items!', 'warning');
			return;
		}
	...
	async processPayments() {
		if (this.OrderLines.length > 0 || this.AllowSendOrder == true) {
			this.env
				.showPrompt('Update the order and proceed to payment!', null, 'Announcement', 'Update', 'No')
				.then((_) => {
					this.sendOrder();
	...
	async notifyOrder(data) {
		...
		//     if(index != -1){
		//         await this.getOrdersOfTable(this.idTable);
		//         this.env.showAlert2("There is a new order on this table","Check the order",'Announcement');
		//     }
		// }
	}
	private notifyFromCustomer(data) {
		...
		if (index != -1) {
			if (!this.IsMyHandle) {
				this.env.showAlert(
					'A customer has already ordered at this table. If you are not with them, please press the bell below to call staff',
					'Check the order',
					'Announcement'
				);
				this.id = data.id;
	...
	async notifyFromStaff() {
		...
		if (this.item.Status == 'Canceled') {
			this.env.showAlert('This order was cancelled!');
		}
		if (this.item.Status == 'Confirmed') {
			this.env.showAlert('The order was confirmed');
			this.playAudio('Order');
		}
		// if(this.item.Status == "Scheduled"){
		//     this.env.showAlert2("Món bạn vừa đặt đã được chuyển Bar/Bếp",null,'Announcement');
		//     //this.playAudio("Order");
		// }
		if (this.item.Status == 'Picking') {
			this.env.showAlert('The dish was prepared');
		}
		if (this.item.Status == 'Delivered') {
			this.env.showAlert('Enjoy your meal');
		}
	...
	async reloadTable(IDTable) {
		await this.commonService
			.connect('GET', 'POS/ForCustomer/Table/' + IDTable, this.query)
			.toPromise()
			.then((result: any) => {
				this.Table = result;
			});
	...
	private getMenu() {
		let apiPath = {
			method: 'GET',
			url: function () {
				return ApiSetting.apiDomain('POS/ForCustomer/Menu');
			},
		};
	...
	private getTable() {
		let apiPath = {
			method: 'GET',
			url: function (id) {
				return ApiSetting.apiDomain('POS/ForCustomer/Table/') + id;
			},
		};
		return new Promise((resolve, reject) => {
			this.commonService
				.connect(apiPath.method, apiPath.url(this.idTable), this.query)
				.toPromise()
				.then((result: any) => {
					resolve(result);
				})
	...
	private getDeal() {
		let apiPath = {
			method: 'GET',
			url: function () {
				return ApiSetting.apiDomain('POS/ForCustomer/Deal');
			},
		};
		return new Promise((resolve, reject) => {
			this.commonService
	...
	private getBranch(id) {
		let apiPath = {
			method: 'GET',
			url: function (id) {
				return ApiSetting.apiDomain('POS/ForCustomer/Branch/') + id;
			},
		};
		return new Promise((resolve, reject) => {
			this.commonService
				.connect(apiPath.method, apiPath.url(id), this.query)
				.toPromise()
				.then((result: any) => {
					this.Branch = result;
					resolve(result);
	...
	async sendOrder() {
		if (this.Table.IsAllowCustomerOrder == true) {
			...
			this.env.showMessage('Sorry, this table has not been activated for ordering', 'warning');
			return false;
		}
	}
	...
	callOrder() {
		if (this.item.Id) {
			...
			this.commonService
				...
				.toPromise()
				.then((result) => {
					this.env.showMessage('Ordered', 'success');
				})
				.catch((err) => {
					console.log(err);
				});
	...
	callToPay() {
		if (this.item.Id) {
			...
			this.commonService
				...
				.toPromise()
				.then((result) => {
					this.env.showMessage('Asked for the bill', 'success');
				})
				.catch((err) => {
					console.log(err);
				});
	...
	callStaff() {
		if (this.item.Id) {
			...
			this.commonService
				...
				.toPromise()
				.then((result) => {
					this.env.showMessage('Served', 'success');
				})
				.catch((err) => {
					console.log(err);
				});
	...
	async helpOrder(status) {
		...
		let message;
		if (status == 'Done') {
			subHeader = 'The order was completed';
		...
		this.env.showAlert(message, subHeader, 'Announcement');
	}
	...
	async checkOrderOfTable(IDTable) {
		let apiPath = {
			method: 'GET',
			url: function (id) {
				return ApiSetting.apiDomain('POS/ForCustomer/OrderOfTable/') + id;
			},
		};
		await this.commonService
			.connect(apiPath.method, apiPath.url(IDTable), this.query)
			.toPromise()
			.then((result) => {
				if (result) {
					this.id = result;
	...
	async getOrdersOfTable(IDTable) {
		await this.commonService
			.connect('GET', 'POS/ForCustomer/OrdersOfTable/' + IDTable, null)
			.toPromise()
			.then((result) => {
				if (result) {
	...
	async getChildrenOrder(IDParent) {
		await this.commonService
			.connect('GET', 'POS/ForCustomer/ChildrenOrder/' + IDParent, null)
			.toPromise()
			.then((result) => {
	...
	async getParentOrder(IDParent) {
		await this.commonService
			.connect('GET', 'POS/ForCustomer/ParentOrder/' + IDParent, null)
			.toPromise()
			.then((result) => {
