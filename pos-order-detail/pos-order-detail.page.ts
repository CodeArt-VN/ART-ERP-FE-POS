...
export class POSOrderDetailPage extends PageBase {
  ...
  async openCancellationReason(line = null, quantity = null) {
    ...
    if (role == 'confirm') {
      ...
      if (!line) {
        this.env
          .showPrompt('Bạn chắc muốn hủy đơn hàng này?', null, 'Hủy đơn hàng')
          .then((_) => {
        ...
        this.env
          .showPrompt('Bạn chắc muốn xóa / giảm số lượng sản phẩm này?', null, 'Xóa sản phẩm')
          .then((_) => {
            let publishEventCode = this.pageConfig.pageName;
            if (this.submitAttempt == false) {
              this.submitAttempt = true;
              ...
              if (result.content.indexOf('VCARD;') != -1) {
                ...
                if (currentTimeFrom <= QRGenTime && QRGenTime <= currentTimeTo) {
                  ...
                  this.env.showMessage(
                    'Mã đã hết hạn, vui lòng lấy lại mã nhân viên mới! Thời gian tạo mã QR: ' + QRGenTime,
                    'danger',
                  );
