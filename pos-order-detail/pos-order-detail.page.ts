...
export class POSOrderDetailPage extends PageBase {
  ...
  private notifySupport(data) {
    ...
    if (this.env.selectedBranch == value.IDBranch) {
      ...
      this.env.showTranslateMessage('Khách bàn {{value}} yêu cầu phục vụ', 'warning',value.Tables[0].TableName );
  ...
  private notifyCallToPay(data) {
    ...
    if (this.env.selectedBranch == value.IDBranch) {
      ...
      this.env.showTranslateMessage('Khách bàn {{value}} yêu cầu tính tiền', 'warning',value.Tables[0].TableName);
      let url = 'pos-order/' + data.id + '/' + value.Tables[0].IDTable;
    ...
    if (!line) {
      ...
      if (line.Quantity > 0 && line.Quantity + quantity < line.ShippedQuantity) {
        if (this.pageConfig.canDeleteItems) {
          this.env
            .showPrompt2(
              'Item này đã chuyển Bar/Bếp, bạn chắc muốn giảm số lượng sản phẩm này?',
              item.Name,
              'Xóa sản phẩm',
            )
            .then((_) => {
          ...
          this.env.showTranslateMessage('Item đã chuyển Bar/Bếp');
          return;
        }
        ...
        if (line.Status == 'New') {
          ...
          if (this.pageConfig.canDeleteItems) {
            ...
            this.env.showTranslateMessage('Tài khoản chưa được cấp quyền xóa sản phẩm!', 'warning');
          }
        }
      }
    }
  ...
  async openCancellationReason(line = null, quantity = null) {
    ...
    if (this.item.Received > 0) {
      this.env.showTranslateMessage(
        'Đơn hàng đã thanh toán không thể hủy, vui lòng hoàn tiền lại để hủy đơn hàng này!',
        'warning',
      );
      return false;
    }
    ...
    if (role == 'confirm') {
      ...
      if (!line) {
        ...
        this.env
          .showPrompt2('Bạn có chắc muốn xóa / giảm số lượng sản phẩm này?', null, 'Xóa sản phẩm')
          .then((_) => {
            let publishEventCode = this.pageConfig.pageName;
            if (this.submitAttempt == false) {
              this.submitAttempt = true;
  ...
  saveOrderData() {
    ...
    this.env
      .showPrompt2(message, null, 'Thông báo')
      .then(async (_) => {
        if (this.item.Id) {
  ...
  async sendKitchenEachItem() {
    return new Promise(async (resolve, reject) => {
      ...
      const IDItemGroupList = [
        ...new Map(
          this.printData.undeliveredItems.map((item: any) => [item['_item']['IDItemGroup'], item._item.IDItemGroup]),
        ).values(),
      ];
      for (let index = 0; index < IDItemGroupList.length; index++) {
        ...
        if (element == 193) {
          ...
          const newKitchenList2 = [
            ...new Map(
              ItemsForKitchen.map((item: any) => [item['_item']['Kitchen']['Name'], item._item.Kitchen]),
            ).values(),
          ];
  ...
  getQRPayment() {
    if (this.paymentList.length) {
      ...
      let payment = this.paymentList.find(
        (p) => p.IncomingPayment.Remark == 'VietQR-AutoGen' && p.IncomingPayment.Status == 'Processing',
      )?.IncomingPayment;
  ...
  GenQRCode(code) {
    ...
    if (
      this.pageConfig.systemConfig.POSEnableTemporaryPayment &&
      this.pageConfig.systemConfig.POSEnablePrintTemporaryBill
    ) {
  ...
  private calcOrder() {
    ...
    for (let line of this.item.OrderLines) {
      ...
      line.CalcOriginalTotalAdditions =
        line.OriginalTotalAfterDiscount * (line._serviceCharge / 100.0) * (1 + line.TaxRate / 100.0);
    ...
    this.item.OriginalTotalDiscountPercent = (
      (this.item.OriginalTotalDiscount / this.item.OriginalTotalBeforeDiscount) *
      100.0
    ).toFixed(0);
    this.item.OriginalTaxPercent = (
      ((this.item.OriginalTax + this.item.AdditionsTax) /
        (this.item.OriginalTotalAfterDiscount + this.item.AdditionsAmount)) *
      100.0
    ).toFixed(0);
    this.item.CalcOriginalTotalAdditionsPercent = (
      (this.item.CalcOriginalTotalAdditions / this.item.OriginalTotalAfterTax) *
      100.0
    ).toFixed(0);
    this.item.AdditionsAmountPercent = (
      (this.item.AdditionsAmount / this.item.OriginalTotalAfterDiscount) *
      100.0
    ).toFixed(0);
    this.item.OriginalDiscountFromSalesmanPercent = (
      (this.item.OriginalDiscountFromSalesman / this.item.CalcTotalOriginal) *
      100.0
    ).toFixed(0);
    this.item.Debt = Math.round(
      this.item.CalcTotalOriginal - this.item.OriginalDiscountFromSalesman - this.item.Received,
    );
  }
            ...
            this.menuProvider
              ...
              .then((resp) => {
                ...
                menuList.forEach((m: any) => {
                  m.menuImage =
                    environment.posImagesServer + (m.Image ? m.Image : 'assets/pos-icons/POS-Item-demo.png');
                  m.Items.forEach((i) => {
                    i.imgPath =
                      environment.posImagesServer + (i.Image ? i.Image : 'assets/pos-icons/POS-Item-demo.png');
                  });
                });
  ...
  setOrderValue(data, instantly = false, forceSave = false) {
    ...
    if (
      (this.item.OrderLines.length || this.formGroup.controls.DeletedLines.value.length) &&
      this.pageConfig.systemConfig.IsAutoSave
    ) {
  ...
  savedChange(savedItem?: any, form?: FormGroup<any>): void {
    ...
    if (
      savedItem.Status == 'TemporaryBill' &&
      this.pageConfig.systemConfig.POSEnableTemporaryPayment &&
      this.pageConfig.systemConfig.POSEnablePrintTemporaryBill
    ) {
      this.sendPrint();
        ...
        .then((result: any) => {
          if (result['count'] > 0) {
            ...
            this.Staff.avatarURL =
              environment.staffAvatarsServer + this.item._Customer.Code + '.jpg?t=' + new Date().getTime();
          }
        });
    }
  }
  discountFromSalesman(line, form) {
    ...
    if (OriginalDiscountFromSalesman > line.CalcTotalOriginal) {
      this.env.showTranslateMessage('Số tiền tặng không lớn hơn trị giá sản phẩm!', 'danger');
      return false;
    }
    this.setOrderValue(
      {
      ...
      this.commonService
        ...
        .then((result: any) => {
          ...
          this.paymentList.forEach((e) => {
            ...
            e.IncomingPayment.TypeText = lib.getAttrib(
              e.IncomingPayment.Type,
          ...
          if (
            this.pageConfig.systemConfig.POSSettleAtCheckout &&
            Math.abs(this.item.Debt) < 10 &&
  ...
  doneOrder() {
    ...
    if (this.printData.undeliveredItems.length > 0) {
      ...
      this.env.showPrompt2({code:message,value:{value:this.item.Tables[0],value1:this.printData.undeliveredItems.length}}, null, 'Thông báo', 'GỬI', null).then((_) => {
                ...
                if (currentTimeFrom <= QRGenTime && QRGenTime <= currentTimeTo) {
                  setTimeout(() => {
                    ...
                    this.contactProvider.read({ Code: StaffCode, Take: 20 }).then((resp) => {
                      ...
                      this.env.showTranslateMessage('Quét thành công! Họ và Tên: {{value}}',null,address['Name']);
