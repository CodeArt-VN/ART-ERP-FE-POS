...
@Component({
    selector: 'app-pos-intro-modal',
    templateUrl: './pos-intro-modal.page.html',
    styleUrls: ['./pos-intro-modal.page.scss'],
})
export class POSIntroModalPage extends PageBase {
    @Input() selectedOrder;
    @Input() orders;
    currentTable;
    newTable = {
    ...
    SlidingCard = [
      {
        ...
        Header: 'Welcome',
        Remark: "Chào mừng bạn đã đến với " + this.currentBranch.Name
      },
      {
        Splash: 'assets/intro-screen/intro-2.jpg',
        Header: 'Fast Order',
        Remark: this.dummyRemark
      },
      {
        Splash: 'assets/undraw_street_food_hm5i.svg',
        Header: 'Be On Time',
        Remark: this.dummyRemark
      }
    ]
    ...
    constructor(
        ...
        this.currentBranch = this.env.branchList.find(i => i.Id == this.env.selectedBranch);
        debugger
    }
    ...
    closeWelcome() {
        this.currentTable; //This can be get when user scan a specific Table QR Code.
        // Assuming that in a QR will contain an ID for that table. 
        // Also make customer to be "Khách lẻ" at default.

        this.modalController.dismiss([this.currentTable]);
    }
